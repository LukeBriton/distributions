...
- **OCR-Nougat**: trying to reconstruct the book with better specialized OCR into markdown (then maybe to pdf).
    ...
    - Single-GPU, Single Batch: taking less than 2GiB VRAM (2134MiB for nougat-base), and about 36 min for the 148 pages on a single RTX 3090.
    - Multi-GPU, Large Batch: e.g. `accelerate launch --num_processes 2 nougat_multi_gpu.py "distributions_fixed.pdf" --out distributions-postprocessed.md --batch 15`, could utilize all VRAM (maximum 24GiB of each) of 2 RTX 3090, taking a little more than 2 min for the same PDF. Even though the usage of stopping_criteria, therefore enforcing return_dict_in_generate and output_scores to be true, makes possible maximum batch much less, still it accelerates more.    
    PS: One can use `export PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True"` before running it to make better use of VRAM accordng to `If reserved but unallocated memory is large try setting PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True to avoid fragmentation.`

    Even though the output needs much proofreading (strange repetitions, ignorance of commutative diagrams), it would still lessen one's burden to type in numerous $\LaTeX$ formulae.    
